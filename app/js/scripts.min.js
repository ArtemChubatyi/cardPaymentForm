function GetPaymentForm() {
	...
	const getSuccessfulScreen = (rootHeight, value) => {
		...
		document.querySelector('.close').addEventListener('click', reload, false);

	}

	const getLoader = (rootHeight) => {
	...
	const mainListener = (event) => {
		...
		if (target === editButton && amountField.disabled === true) {
			...
			setTimeout(getSuccessfulScreen, 2000, rootPreviousHeight, amountField.value);
		}
	}
